...
function validarTextArea() {
  if (textArea.value.trim() === "") {
    iconosCorrectos[3].classList.remove("input__correecto");
    iconosIncorrectos[3].classList.remove("input__Incorreecto");
  ...
  if (expresiones.mensaje.test(textArea.value)) {
    ...
    leyendaError[3].textContent =
      "Su mensaje es demasiado extenso. Por favor, reduzcalo a máximo 500 caracteres";
  };
};
...
formulario.addEventListener("submit", (e) => {
  ...
  inputs.forEach((input, index) => {
    ...
    if (iconosIncorrectos[index].classList.contains("input__incorrecto")) {
      //Si el campo no cumple con la expresión regular, se muestra el mensaje de error
      asterisco[index].style.display = "inline-block";
      hayErrores = true;
    }
  });
  ...
  if (!hayErrores) {
    console.log("¡LA VARIABLE HAY ERRORES ES: " + hayErrores);
    formulario.submit();
  } else {
    console.log("¡LA VARIABLE HAY ERRORES ES: " + hayErrores);
  }
